    ...
    <div class="banner">
        <ul>
          <li :class="key==ban?'active':''" v-for="(item,key) in banner">
            <router-link :to="{name:'songPlay',params:{id:item.encodeId}}">
              <img :src="item.imageUrl+'?param=1200x400'" alt="">
            </router-link>
          </li>
        </ul>
      <p class="cont">
        <span @mouseenter="ban=key" :class="key==ban?'active':''" v-for="(item,key) in banner"></span>
      </p>
    </div>
    <div class="new_n">
    ...
    $.get(that.url + '/banner',
      function (data) {
        ...
        setInterval(function () {
          that.ban++
          if (that.ban >= data.banners.length) that.ban = 0
        }, 4000)
      }
    )
  }
